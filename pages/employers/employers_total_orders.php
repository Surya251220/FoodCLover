...
<body>
  ...
  if (!empty($search)) {
    $sql = "SELECT c.full_name, o.order_id, o.reservation_id, o.table_number, r.status, o.created_at, o.updated_at, o.guest_id, g.name, g.verification_number
        FROM orders o
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (isset($row["reservation_id"]) && isset($row["table_number"]) && isset($row["full_name"]) || isset($row["name"])) {
      echo '<form method="get" action="singleOrder.php">
                ...
                <button style="margin-top: 10px" class="signup_button" type="submit">See more details</button>
              </form>';
      ...
      echo '<form method="get" action="customerReceipts.php">
                ...
                <input type="hidden" name="customer_id" value="' . $row["full_name"] . '">
                <input type="hidden" name="guest_id" value="' . $row["name"] . '">
				<button style="margin-top: 10px" class="signup_button" type="submit">Receipts</button>
			  </form>';
    } else {
      echo '-';
    }
