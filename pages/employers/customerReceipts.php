...
<body>
  ...
  if (isset($_GET['full_name'])) {
    ...
    $where_condition = "customers.full_name = '$customer_name'";
  } elseif (isset($_GET['name'])) {
    $guest_name = $_GET['name'];
    $where_condition = "guests.name = '$guest_name'";
  } else {
    $where_condition = "1"; // return all records
  }
  $reservation_id = $_GET['reservation_id'];
  $table_number = $_GET['table_number'];

  $sql = "SELECT *, customers.email AS customer_email, guests.email AS guest_email, orders.created_at AS customer_created_at, orders.created_at AS guest_created_at
FROM orders
  ...
  if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      if (isset($row['full_name'])) {
        ...
        $reservation_date = $row["customer_created_at"];
      } elseif (isset($row['name'])) {
        $customer_id = $row['guest_id'];
        $customer_name = $row['name'];
        $customer_email = $row["guest_email"];
        $customer_phone = $row["phone_number"];
        $reservation_date = $row["guest_created_at"];
      } else {
        $customer_name = '';
        $customer_email = '';
        $customer_phone = '';
