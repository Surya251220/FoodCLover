...
if (isset($_POST['generate-submit'])) {
    ...
    if (isset($_SESSION['email'])) {
        ...
        header("Location: ../pages/customers/customersTwoStep.php?error=$errorParam");
        exit();
    }
    ...
    $updateCodeQuery = $conn->prepare("UPDATE customers SET two_step_verification = ? WHERE email = ?");
    ...
    if (isset($_SESSION['email'])) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            header("Location: ../pages/customers/customersTwoStep.php?error=sqlerror");
            exit();
        } else {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($verificationCode == $storedCode) {
                    ...
                    header("Location: ../pages/customers/customer_menu.php?error=success");
                    exit();
                } else {
                    header("Location: ../pages/customers/customersTwoStep.php?error=incorrectcode");
                    exit();
                }
            } else {
                header("Location: ../pages/customers/customersTwoStep.php?error=nouser");
                exit();
            }
        }
    } else {
