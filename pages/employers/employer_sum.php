...
if (isset($_GET['customer_id']) && $_GET['customer_id'] !== '') {
    ...
    $guest_id = null;
} elseif (isset($_GET['guest_id']) && $_GET['guest_id'] !== '') {
...
$sql = "SELECT * FROM cart WHERE ((customer_id = ? AND reservation_id = ?) OR (guest_id = ? AND reservation_id = ?)) AND status = 'unconfirmed'";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "iiii", $customer_id, $reservation_id, $guest_id, $reservation_id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
...
if (isset($_POST['delete'])) {
    ...
    header("Location: employer_sum.php?reservation_id=" . $_POST['reservation_id'] . "&table_number=" . $_POST['table_number'] . "&customer_id=" . $_POST['customer_id'] . "&guest_id=" . $_POST['guest_id']);
    exit();
}
...
echo "<h2>Employer Summary</h2>";
echo "<form class='order-summary' method='post' action='employer_sum.php?reservation_id=$reservation_id&table_number=$table_number&customer_id=$customer_id&guest_id=$guest_id'>";
echo "<table>";
...
echo "<input type=\"hidden\" name=\"table_number\" value=\"" . $table_number . "\">";
